...
def detect_face_part(face_list, video, number, count, sess, detection_graph, category_index) :
        ...
        if (int(video.get(1)) % num == c ):
            ...
            for j in range(len(sess)):
                ...
                if left != 0 and height>0 and width>0:
                    ...
                    # 탐지 결과 그리기 (일명 '결과 시각화')
                    label_str, left, right, top, bottom = vis_util.visualize_boxes_and_labels_on_image_array(
                        image,
                        np.squeeze(boxes),
                ...
                # 박스 확인용 이미지 출력
                output_dir = './test_videos/face_part/'+video_name+"/"+model_name+'/'
                if not os.path.exists('./test_videos/face_part/'+video_name):
                    os.mkdir('./test_videos/face_part/'+video_name)
                    print(str('./test_videos/face_part/'+video_name))
    ...
    return load_model_time, eye_list, nose_list, mouth_list